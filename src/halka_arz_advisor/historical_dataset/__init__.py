...
1. **A fact only enters a snapshot's features/decision if its own
   ...
   :mod:`halka_arz_advisor.issuer_ir.ingest`) and is never used here.
   :class:`halka_arz_advisor.spk.models.SpkIpoRecord` (SPK's
   *completed*-IPO record) is used only for canonical identity and for
   the outcome label's own IPO dates — never as a feature source for
   the reconstructed decision, since this project has no per-record
   publish timestamp to prove any of its fields were knowable before
   the offer completed (every catalog feature that reads from it is
   already tagged ``offer_timing="post_offer"`` — see
...
from .cutoff import CutoffResolution, resolve_decision_cutoff
