...
def format_explanation(result: DecisionResult) -> str:
    ...
    total_str = f"{result.total_score:.1f}" if result.total_score is not None else "yok"
    lines.append(f"Toplam skor: {total_str} / 100")
    ...
    for category in result.category_scores:
        ...
        lines.append(f"  - {category.category}: {score_str} / 100 (kapsam: {category.coverage:.0%}, durum: {category.status})")
